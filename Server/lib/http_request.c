...
struct http_request *http_request_new() {
    ...
    httpRequest->request_headers = malloc(sizeof(struct request_header) * INIT_REQUEST_HEADER_SIZE);
    httpRequest->request_headers_number = 0;
    httpRequest->body = buffer_new();
    return httpRequest;
}
...
void http_request_reset(struct http_request *httpRequest) {
    ...
    httpRequest->url = NULL;
    httpRequest->request_headers_number = 0;
    buffer_data_reset(httpRequest->body);
}
