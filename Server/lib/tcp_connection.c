...
int handle_write_to_target(void *data) {
    struct tcp_connection *tcpConnection = (struct tcp_connection *) data;
    struct event_loop *eventLoop = tcpConnection->eventLoop;
    assertInSameThread(eventLoop);
    
    struct buffer *output_buffer = tcpConnection->output_buffer;
    struct channel *channel2 = tcpConnection->channel2;
    yolanda_msgx("handle_write_to_target");
    ssize_t nwrited = write(channel2->fd, output_buffer->data + output_buffer->readIndex,
                            buffer_readable_size(output_buffer));
    if (nwrited > 0) {
        //已读nwrited字节
        output_buffer->readIndex += nwrited;
        //如果数据完全发送出去，就不需要继续了
        if (buffer_readable_size(output_buffer) == 0) {
            event_loop_remove_channel_event(eventLoop, channel2->fd, channel2);
        }
        //回调writeCompletedCallBack
        if (tcpConnection->writeCompletedCallBack != NULL) {
            tcpConnection->writeCompletedCallBack(tcpConnection);
...
tcp_connection_new(int connected_fd, struct event_loop *eventLoop,
    ...
    tcpConnection->userId = -1;
    tcpConnection->targetFd = -1;
    ...
    // default send data to source userfd;
    tcpConnection->channel2 = channel_new(connected_fd, EVENT_WRITE, NULL , handle_write_to_target, tcpConnection);
...
void tcp_connection_set_targetfd(struct tcp_connection *tcpConnection, int fd){
    tcpConnection->targetFd = fd;
}
...
int tcp_connection_send_data(struct tcp_connection *tcpConnection, void *data, int size) {
    ...
    if(tcpConnection->targetFd != -1){
        tcpConnection->channel2->fd = tcpConnection->targetFd;
        channel = tcpConnection->channel2;
        write_enabled = true;
    }else{
        channel = tcpConnection->channel;
        write_enabled = !channel_write_event_is_enabled(channel);
    }
    
    struct buffer *output_buffer = tcpConnection->output_buffer;
    yolanda_msgx("tcp_connection_send_data");

    //先往套接字尝试发送数据
    if ( write_enabled && buffer_readable_size(output_buffer) == 0) {
        yolanda_msgx("tcp_connection_send_data->write");
    ...
    if (!fault && nleft > 0) {
        ...
        if(tcpConnection->targetFd == -1){
            ...
            yolanda_msgx("add_channel2");
            //向target发送数据, channel2
            event_loop_add_channel_event(tcpConnection->eventLoop, tcpConnection->channel2->fd, tcpConnection->channel2);
        }
