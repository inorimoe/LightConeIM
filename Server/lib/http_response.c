...
void http_response_encode_buffer(struct http_response *httpResponse, struct buffer *output) {
    ...
    if (httpResponse->keep_connected) {
        ...
        if(httpResponse->body != NULL){
            bodyLen = strlen(buffer_get_string(httpResponse->body));
        }
    ...
    buffer_append_string(output, "\r\n");
    buffer_append_string(output, buffer_get_string(httpResponse->body));
    buffer_append_char(output, '\0');
}
...
void http_response_clear(struct http_response *httpResponse) {
    if (httpResponse->response_headers != NULL) {
        for (int i = 0; i < httpResponse->response_headers_number; i++) {
            free(httpResponse->response_headers[i].key);
            free(httpResponse->response_headers[i].value);
        }
        free(httpResponse->response_headers);
    }
    if ( httpResponse->body != NULL ){
        buffer_free(httpResponse->body);
    }
    free(httpResponse);
}
...
void http_response_reset(struct http_response *httpResponse){ 
    ...
    httpResponse->response_headers_number = 0;
    httpResponse->keep_connected = 0;
    buffer_data_reset(httpResponse->body);
}
