...
//    te_font.setLetterSpacing(QFont::AbsoluteSpacing, 0);             //设置字间距为3像素 //设置字间距像素值
    ...
    //m_leftPixmap = QPixmap(getImagePath() + "CustomerService.png");
    //m_rightPixmap = QPixmap(getImagePath() + "Customer Copy.png");
    m_leftPixmap = QPixmap(getImagePath() + "itemDefaultImage.png");
    m_rightPixmap = QPixmap(getImagePath() + "itemDefaultImage.png");
...
void ChatMsgItem::setText(QString text, QString time, QSize allSize, ChatMsgItem::User_Type userType)
{
    m_msg = text;
    m_userType = userType;
    m_time = time;
...
QSize ChatMsgItem::fontRect(QString str)
{
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
...
QSize ChatMsgItem::getRealString(QString src)
{
